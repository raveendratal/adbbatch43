...
-- MAGIC |----|-----
-- MAGIC | Notebook | SQL DRL GROUPING SETS ROLLUP CUBE Statement details 
...
-- MAGIC %md
-- MAGIC ### GROUPING SETS, CUBE, ROLLUP
...
-- MAGIC %md
-- MAGIC * Alternate syntax for __`GROUPING SETS`__ in which both __`GROUP BY`__ and __`GROUPING SETS`__
-- MAGIC * specifications are present.

-- COMMAND ----------

SELECT sum(quantity),city AS sum FROM dealer GROUP BY city --, car_model
...
-- ROLLUP Equivalent GROUP BY GROUPING SETS ((city, car_model), (city), ())
-- CUBE Equivalent GROUP BY GROUPING SETS ((city, car_model), (city), (car_model), ())
SELECT city, car_model, sum(quantity) AS sum FROM dealer
